...
export default function OrdersPage() {
  ...
  const stats = [
    ...
    { title: 'Pending', value: orders.filter(o => o.status === 'pending').length, icon: Clock, color: 'bg-yellow-500', change: 8.7, trend: 'up' },
    { title: 'In Progress', value: orders.filter(o => o.status === 'in_progress').length, icon: TrendingUp, color: 'bg-purple-500', change: 15.3, trend: 'up' },
    { title: 'Revenue', value: orders.reduce((sum, o) => sum + o.amount, 0), icon: DollarSign, color: 'bg-green-500', change: 22.1, trend: 'up' }
  ]
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {stats.map((stat, index) => {
            const StatusIcon = getStatusIcon(stat.title.toLowerCase())
            return (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 20 }}
