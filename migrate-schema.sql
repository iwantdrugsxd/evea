-- =============================================================================
-- EVEA DATABASE MIGRATION SCRIPT
-- This script applies the complete database schema to an existing Supabase database
...
DO $$ BEGIN
    ...
    ALTER TABLE vendors ADD COLUMN IF NOT EXISTS business_logo_url TEXT;
    ALTER TABLE vendors ADD COLUMN IF NOT EXISTS industry VARCHAR(100);
    ...
    ALTER TABLE vendors ADD COLUMN IF NOT EXISTS annual_revenue DECIMAL(15,2);
    ALTER TABLE vendors ADD COLUMN IF NOT EXISTS website_url TEXT;
    ALTER TABLE vendors ADD COLUMN IF NOT EXISTS social_media_links JSONB;
    ALTER TABLE vendors ADD COLUMN IF NOT EXISTS contact_person_name VARCHAR(255);
    ALTER TABLE vendors ADD COLUMN IF NOT EXISTS contact_person_designation VARCHAR(100);
    ALTER TABLE vendors ADD COLUMN IF NOT EXISTS contact_person_phone VARCHAR(20);
    ALTER TABLE vendors ADD COLUMN IF NOT EXISTS contact_person_email VARCHAR(255);
    ...
    ALTER TABLE vendors ADD COLUMN IF NOT EXISTS service_coverage_areas TEXT[];
    ...
    ALTER TABLE vendors ADD COLUMN IF NOT EXISTS total_revenue DECIMAL(15,2) DEFAULT 0;
...
-- Add missing columns to existing vendor_services table
DO $$ BEGIN
    ALTER TABLE vendor_services ADD COLUMN IF NOT EXISTS service_name VARCHAR(255) NOT NULL DEFAULT 'Service';
    ALTER TABLE vendor_services ADD COLUMN IF NOT EXISTS service_specializations TEXT[];
    ALTER TABLE vendor_services ADD COLUMN IF NOT EXISTS service_areas TEXT[];
    ALTER TABLE vendor_services ADD COLUMN IF NOT EXISTS equipment_available TEXT[];
    ALTER TABLE vendor_services ADD COLUMN IF NOT EXISTS team_size INTEGER;
    ALTER TABLE vendor_services ADD COLUMN IF NOT EXISTS experience_years INTEGER;
    ALTER TABLE vendor_services ADD COLUMN IF NOT EXISTS certifications TEXT[];
    ALTER TABLE vendor_services ADD COLUMN IF NOT EXISTS awards TEXT[];
    ALTER TABLE vendor_services ADD COLUMN IF NOT EXISTS portfolio_videos TEXT[];
    ALTER TABLE vendor_services ADD COLUMN IF NOT EXISTS competitive_analysis TEXT;
    ALTER TABLE vendor_services ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT TRUE;
    ALTER TABLE vendor_services ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
EXCEPTION
    WHEN duplicate_column THEN null;
END $$;
...
-- Add missing columns to existing vendor_cards table
DO $$ BEGIN
    ALTER TABLE vendor_cards ADD COLUMN IF NOT EXISTS price_range_min DECIMAL(10,2);
    ALTER TABLE vendor_cards ADD COLUMN IF NOT EXISTS price_range_max DECIMAL(10,2);
    ALTER TABLE vendor_cards ADD COLUMN IF NOT EXISTS availability_days TEXT[];
    ALTER TABLE vendor_cards ADD COLUMN IF NOT EXISTS availability_hours JSONB;
    ALTER TABLE vendor_cards ADD COLUMN IF NOT EXISTS response_time_hours INTEGER;
    ALTER TABLE vendor_cards ADD COLUMN IF NOT EXISTS total_orders INTEGER DEFAULT 0;
    ALTER TABLE vendor_cards ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
EXCEPTION
    WHEN duplicate_column THEN null;
END $$;
...
DO $$ BEGIN
    ALTER TABLE orders ADD COLUMN IF NOT EXISTS vendor_card_id UUID REFERENCES vendor_cards(id);
    ALTER TABLE orders ADD COLUMN IF NOT EXISTS event_start_time TIME;
    ALTER TABLE orders ADD COLUMN IF NOT EXISTS event_end_time TIME;
    ALTER TABLE orders ADD COLUMN IF NOT EXISTS event_type VARCHAR(100);
    ALTER TABLE orders ADD COLUMN IF NOT EXISTS service_requirements TEXT;
    ALTER TABLE orders ADD COLUMN IF NOT EXISTS special_requests TEXT;
    ALTER TABLE orders ADD COLUMN IF NOT EXISTS base_amount DECIMAL(10,2);
...
DO $$ BEGIN
    ALTER TABLE order_items ADD COLUMN IF NOT EXISTS item_name VARCHAR(255) NOT NULL DEFAULT 'Item';
    ALTER TABLE order_items ADD COLUMN IF NOT EXISTS item_description TEXT;
EXCEPTION
    WHEN duplicate_column THEN null;
END $$;
...
DO $$ BEGIN
    ...
    ALTER TABLE notifications ADD COLUMN IF NOT EXISTS related_entity_type VARCHAR(100);
...
CREATE TABLE IF NOT EXISTS vendor_business_hours (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    vendor_id UUID REFERENCES vendors(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_vendors_average_rating ON vendors(average_rating);
...
CREATE INDEX IF NOT EXISTS idx_vendor_cards_vendor_id ON vendor_cards(vendor_id);
CREATE INDEX IF NOT EXISTS idx_vendor_cards_category_id ON vendor_cards(category_id);
CREATE INDEX IF NOT EXISTS idx_vendor_cards_base_price ON vendor_cards(base_price);
CREATE INDEX IF NOT EXISTS idx_vendor_cards_featured ON vendor_cards(featured);
CREATE INDEX IF NOT EXISTS idx_vendor_cards_average_rating ON vendor_cards(average_rating);
CREATE INDEX IF NOT EXISTS idx_vendor_cards_is_active ON vendor_cards(is_active);
...
DROP TRIGGER IF EXISTS update_vendor_cards_updated_at ON vendor_cards;
...
DROP TRIGGER IF EXISTS update_vendor_verification_documents_updated_at ON vendor_verification_documents;
...
SELECT 'Wedding Services', 'wedding-services', 'Complete wedding planning and services', true
WHERE NOT EXISTS (SELECT 1 FROM categories WHERE slug = 'wedding-services');

INSERT INTO categories (name, slug, description, is_active) 
SELECT 'Photography', 'photography', 'Professional photography services', true
WHERE NOT EXISTS (SELECT 1 FROM categories WHERE slug = 'photography');

INSERT INTO categories (name, slug, description, is_active) 
SELECT 'Videography', 'videography', 'Video recording and editing services', true
WHERE NOT EXISTS (SELECT 1 FROM categories WHERE slug = 'videography');

INSERT INTO categories (name, slug, description, is_active) 
SELECT 'Catering', 'catering', 'Food and beverage services', true
WHERE NOT EXISTS (SELECT 1 FROM categories WHERE slug = 'catering');

INSERT INTO categories (name, slug, description, is_active) 
SELECT 'Decoration', 'decoration', 'Event decoration and styling', true
WHERE NOT EXISTS (SELECT 1 FROM categories WHERE slug = 'decoration');

INSERT INTO categories (name, slug, description, is_active) 
SELECT 'Music & Entertainment', 'music-entertainment', 'Live music and entertainment services', true
WHERE NOT EXISTS (SELECT 1 FROM categories WHERE slug = 'music-entertainment');

INSERT INTO categories (name, slug, description, is_active) 
SELECT 'Transportation', 'transportation', 'Vehicle and transportation services', true
WHERE NOT EXISTS (SELECT 1 FROM categories WHERE slug = 'transportation');

INSERT INTO categories (name, slug, description, is_active) 
SELECT 'Venues', 'venues', 'Event venues and locations', true
WHERE NOT EXISTS (SELECT 1 FROM categories WHERE slug = 'venues');

-- Insert subcategories if they don't exist
INSERT INTO categories (name, slug, description, parent_id, is_active) 
SELECT 'Wedding Photography', 'wedding-photography', 'Specialized wedding photography', 
       (SELECT id FROM categories WHERE slug = 'photography'), true
WHERE NOT EXISTS (SELECT 1 FROM categories WHERE slug = 'wedding-photography');

INSERT INTO categories (name, slug, description, parent_id, is_active) 
SELECT 'Wedding Videography', 'wedding-videography', 'Wedding video services', 
       (SELECT id FROM categories WHERE slug = 'videography'), true
WHERE NOT EXISTS (SELECT 1 FROM categories WHERE slug = 'wedding-videography');

INSERT INTO categories (name, slug, description, parent_id, is_active) 
SELECT 'Wedding Catering', 'wedding-catering', 'Wedding food services', 
       (SELECT id FROM categories WHERE slug = 'catering'), true
WHERE NOT EXISTS (SELECT 1 FROM categories WHERE slug = 'wedding-catering');
